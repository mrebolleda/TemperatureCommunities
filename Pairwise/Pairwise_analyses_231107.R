...
pdf("Pairwise/Output/freq_slow.pdf", width = 6, height = 3.5)
ggplot(meanFreq_Pair_Temp, aes(x=Temperature, y=mean, color=as.factor(Pair)))+
  facet_wrap(1~slow)+
  geom_point()+
...
dt_glu <- data.table(density=rep(0,12),RF_ratio=rep(rf_data_sugs[Carbon=="Glu" & Temp==30,rf],3), Temperature=rep(c(20,25,30), each=4))

pdf("Pairwise/Output/dist_rf.pdf", width = 6, height = 2.5)
ggplot(counts_gr, aes(x=log(RF_ratio)))+
  facet_wrap(~Temperature)+
  geom_density()+
  geom_point(data=dt_glu, aes(x=log(RF_ratio), y=density))+
...
# Calculate mean growth difference across temps
counts_gr[, mean_gdif:=(R23_F23+R28_F28+R30_F30)/3]
...
#### Categorical based on behavior
gr_rf_mean$dif_cat <- "equal"
gr_rf_mean$dif_cat[gr_rf_mean$mean_gdif< -0.05] <- "respirator slow"
gr_rf_mean$dif_cat[gr_rf_mean$mean_gdif> 0.05] <- "fermenter slow"

pdf("Pairwise/Output/grdif_rf.pdf", width = 3, height = 7)
ggplot(gr_rf_mean[dif_cat!="equal",], aes(y=mean_rf, x=dif_cat, shape=as.factor(Temperature)))+
  geom_point(position=position_dodge(width = 0.3), size=1)+
  stat_summary(fun="median", geom="point", size=2, position=position_dodge(width = 0.3) )+
  #xlab("mean(r(R)-r(F)) (/hr)")+
  ylab("log(R/F)")+
  geom_vline(xintercept = 0, linetype = 2)+
  theme_bw()
...
t.test(gr_rf_mean$mean_rf[gr_rf_mean$dif_cat=="fermenter slow"], gr_rf_mean$mean_rf[gr_rf_mean$dif_cat=="respirator slow"])

#### Linear models per temperature
m <- lm(mean_rf~mean_gdif, data=gr_rf_mean[Temperature==20,]) %>% summary
m1 <-lm(mean_rf~mean_gdif, data=gr_rf_mean[Temperature==25,]) %>% summary
m2 <- lm(mean_rf~mean_gdif, data=gr_rf_mean[Temperature==30,]) %>% summary

pdf("Pairwise/Output/residuals_gr.pdf", width = 6, height = 3)
par(mfrow=c(1,3))
plot(gr_rf_mean[Temperature==20,]$mean_gdif, residuals(m), ylab="residuals", xlab="r(R)-r(F) (/hr)")
abline(v=-0.05, lty=2)
abline(v=0.05, lty=2)
plot(gr_rf_mean[Temperature==25,]$mean_gdif, residuals(m1), ylab="residuals", xlab="r(R)-r(F) (/hr)")
abline(v=-0.05, lty=2)
abline(v=0.05, lty=2)
plot(gr_rf_mean[Temperature==30,]$mean_gdif, residuals(m2), ylab="residuals", xlab="r(R)-r(F) (/hr)")
abline(v=-0.05, lty=2)
abline(v=0.05, lty=2)
dev.off()
