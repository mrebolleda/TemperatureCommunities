...
# Assign directory to taxa databases 
# path.taxa <- "Directory to taxonomic databases (full)"
...
taxa <- assignTaxonomy(seqtab.nochim, paste0(path.taxa, "/silva_nr_v132_train_set.fa.gz"), multithread=TRUE)
taxa.print <- taxa # Removing sequence rownames for display only
rownames(taxa.print) <- NULL
head(taxa.print)

write.table(taxa, file= paste0(path.out,"taxa_silva_v132.txt"))

```

# Assign taxonomy using rDP to match with rRNA operon copy numbers
```{r}
taxardp <- assignTaxonomy(seqtab.nochim, refFasta = paste0(path.taxa, "/rdp_train_set_18.fa.gz"), multithread=TRUE)

write.table(taxardp, file = paste0(path.out, "/taxa_rdp.txt"))
``` 
